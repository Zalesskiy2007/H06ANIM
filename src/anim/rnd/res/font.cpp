...
mzgl::font & mzgl::font::FntLoad( const CHAR *FileName, std::string Name )
{
  ...
  fread(tex, 4, W * H, F);
  fclose(F);
  material_pattern *MtlP = anim::Get()->MtlPatCreate(Name, 16, "font");
  mtl = anim::Get()->MtlCreate(Name, vec3(0.3, 0, 0), 
                                               vec3(1, 0, 0), vec3(1, 1, 1),
                                               32, 1, MtlP);
  texture *t = anim::Get()->TxtLoadImg(Name, W, H, 4, tex);
  mtl->AddTexture(t);
  free(tex);
  this->Mtl = mtl;
  for (i = 0; i < 256; i++)
  {
    ...
    FntChars[i] = anim::Get()->PrimCreate<vertex::std>(this->Mtl, topo);
  }
...
VOID mzgl::font::FntDraw( CHAR *Str )
{
  ...
  while (*Str != 0)
  {
    if (*Str == '\n')
      ...
      {
        anim::Get()->Draw(FntChars[(UCHAR)*Str], matr().Translate(Pos));
