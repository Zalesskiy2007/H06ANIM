...
VOID mzgl::render::Init(VOID)
{
  ...
  mzgl::anim::GetRef().Path = Buf;

  Trg = mzgl::anim::Get()->TrgCreate("FBO");
  Lgh = mzgl::anim::Get()->LghCreate("Shadows");
  IsInit = TRUE;
...
*/
mzgl::prim * mzgl::render::LoadModel( const CHAR *Fn, material *Mat )
{
  ...
  return mzgl::anim::Get()->PrimCreate(Mat, topo);
...
VOID mzgl::render::Draw(const prim* Pr, const matr& World)
{
  ...
  if ((loc = glGetUniformLocation(RndProgId, "Time")) != -1)
    glUniform1f(loc, mzgl::anim::Get()->Time);
